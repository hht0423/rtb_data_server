...
public class RtbCacheDao extends AbstractCacheDao{
	...
	private static final String CACHE_RTB_ADSYNC_THIRD_ADIDS_PAGE_PREFIX="cache::rtb::adsync::third::adids::page::";
	...
	public void cacheThirdSourceAdIdsByPage(int spaceId,int page, List<String> adIds){
		if(CollectionUtils.isEmpty(adIds)){
			return ;
		}
		String key=CACHE_RTB_ADSYNC_THIRD_ADIDS_PAGE_PREFIX+spaceId+CACHE_SLIPT+page;
		redisTemplate.opsForList().rightPushAll(key, adIds);
		redisTemplate.boundSetOps(key).expire(getCacheTimeOut(), TimeUnit.SECONDS);
	}
	...
	public List<String> getThirdSourceIdByPage(int spaceId,int page){
		String key=CACHE_RTB_ADSYNC_THIRD_ADIDS_PAGE_PREFIX+spaceId+CACHE_SLIPT+page;
		return redisTemplate.opsForList().range(key, 0, -1);
	}
