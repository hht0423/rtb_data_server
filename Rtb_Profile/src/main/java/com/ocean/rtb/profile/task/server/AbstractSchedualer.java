...
import com.ocean.rtb.profile.task.data.AbstractCacheTask;
import com.ocean.rtb.profile.task.service.ITaskProfileService;
...
public abstract class AbstractSchedualer extends LifeCycle{
	...
	private final BlockingQueue<AbstractCacheTask> queue;
	...
	public void addTask(AbstractCacheTask task){
		queue.add(task);
	}
	public AbstractCacheTask getTask() throws InterruptedException{
		return queue.take();
	}
	public class Invoke extends Thread implements ILifecycleListener{
	  
		private ITaskProfileService syncService;
		
		public Invoke(ITaskProfileService syncService){
			this.syncService=syncService;
		
		}
		private AbstractCacheTask getTask() throws InterruptedException{
		...
		public void run(){
			while(true){
				...
				try{
					AbstractCacheTask task=this.getTask();
					if(task!=null){
